...
## Acceptance Criteria
- [x] Given a valid request, when POSTed then a PENDING charge + a negative CHARGE ledger entry are written in one txn, balance == SUM(ledger) (INV-1/INV-7), 201 returned.
- [x] Given a repeated Idempotency-Key, when POSTed then the original charge is returned with no second entry.
- [x] Given EXEMPT treatment, when created then tax=0, gross=net (INV-7), legal reason present.
...
## Definition of Done
- [x] Integration test: create -> charge+entry+balance; replay; EXEMPT; pnpm typecheck + tests green
